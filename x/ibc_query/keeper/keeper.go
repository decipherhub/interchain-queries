...
func (k Keeper) SetCrossChainQueryResult(ctx sdk.Context, resultQueryPath string, result types.CrossChainQueryResult) {
    ...
    store.Set([]byte(resultQueryPath), bz)
}

// GetCrossChainQueryResult retrieve the CrossChainQueryResult stored in state given the query id
func (k Keeper) GetCrossChainQueryResult(ctx sdk.Context, resultQueryPath string) (types.CrossChainQueryResult, bool) {
    store := prefix.NewStore(ctx.KVStore(k.storeKey), types.QueryResultKey)
    key := []byte(resultQueryPath)
    bz := store.Get(key)
    if bz == nil {
...
func (k Keeper) DeleteCrossChainQueryResult(ctx sdk.Context, resultQueryPath string) {
    store := prefix.NewStore(ctx.KVStore(k.storeKey), types.QueryResultKey)
    store.Delete([]byte(resultQueryPath))
}
